...
<table id="fixed-header" class="table table-bordered dt-responsive nowrap table-striped align-middle" style="width:100%">
    ...
    <tbody>
        @foreach($bonafids as $bonafid)
            <tr id="lcRow{{ $bonafid->id }}">
                <td>{{ $bonafid->id }}</td>
                <td>{{ isset($bonafid->state) ? $bonafid->state : 'N/A' }}</td>
                <td>{{ isset($bonafid->district) ? $bonafid->district : 'N/A' }}</td>
                <td>{{ isset($bonafid->taluka) ? $bonafid->taluka : 'N/A' }}</td>
                <td>{{ isset($bonafid->gram->gram_name) ? $bonafid->gram->gram_name : 'N/A' }}</td>
                <td>{{ isset($bonafid->school_address) ? $bonafid->school_address : 'N/A' }}</td>
                <td>{{ isset($bonafid->student_name) ? $bonafid->student_name : 'N/A' }}</td>
                <td>{{ isset($bonafid->general_register_number) ? $bonafid->general_register_number : 'N/A' }}</td>
                <td>{{ isset($bonafid->religion) ? $bonafid->religion : 'N/A' }}</td>
                <td>{{ isset($bonafid->caste) ? $bonafid->caste : 'N/A' }}</td>
                <td>{{ isset($bonafid->dob) ? $bonafid->dob : 'N/A' }}</td>
                <td>
                    <a href="{{ route('bonafid.edit' , $bonafid->id) }}" class="btn btn-sm btn-primary my-3">Edit</a>
<button class="btn btn-sm btn-success viewCategory my-3 " data-id="{{ $bonafid->id }}">
    <i class="fa fa-eye"></i> View
</button>
<a href="#" class="btn btn-danger btn-sm remove-item-btn my-3" data-id="{{ $bonafid->id }}" data-bs-toggle="modal" data-bs-target="#deleteRecordModal">Delete</a>
<a href="{{ route('bonafid.print', $bonafid->id) }}" class="btn btn-warning btn-sm" target="_blank" onclick="openAndPrintPage(event, '{{ route('bonafid.print', $bonafid->id) }}')">Print</a>
