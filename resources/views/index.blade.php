...
<h4 class="fs-16 mb-1">Good Morning, {{ Auth::user()->name }}, @if(Auth::user()->is_admin != 'admin')({{ $permissions['role_name']}})  @endif </h4><p class ="text-muted mb-0" id="current-time" class="fs-14"></p>
            ...
            <div class="row">
                @if(Auth::user())
                 @if(($permissions['id'] != 1 && $permissions['id'] != 2) || Auth::user()->is_admin == "admin")  
                        ...
                        <div class="card-body">
                            ...
                            <div class="mt-4">
                                ...
                                <h4 class="fs-22 fw-semibold ff-secondary mb-4" style="margin-top:12px">
                                    <span id="categoryCount" class="text-primary">{{$getRegisterGramNew}}</span>
                                </h4>
                                @if($getRegisterGram != 0  &&  $permissions['registered_gram_viewown'] == 2 || $permissions['registered_gram_viewall'] == 2 )
                                    <a href="{{route('register-to-gram.index')}}" class="text-decoration-none text-muted">View All</a>
                                    @else
            ...
            <div class="mt-4">
                ...
                <div class="mb-4">
                    <select id="gramDropdown" class="form-select js-example-basic-single">
                        ...
                        <option value="{{ $gram->gram_name }}">
                            {{ $gram->gram_name }}-{{ $gram->taluka }}-{{ $gram->district }}
                        </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="d-flex justify-content-between align-items-center">
                    ...
                    <div class="text-center mx-2">
                        <h4 class="fs-22 fw-semibold ff-secondary mb-2">
                            <span id="pendingCount" class="text-primary">{{ $pendingCount }}</span>
                        </h4>
                    ...
                    <div class="text-center mx-2">
                        <h4 class="fs-22 fw-semibold ff-secondary mb-2">
                            <span id="completeCount" class="text-success">{{ $completedCount }}</span>
                        </h4>
                    ...
                    <div class="text-center mx-2">
                        <h4 class="fs-22 fw-semibold ff-secondary mb-2">
                            <span id="totalAmount" class="text-danger">{{ $totalAmount }}</span>
                        </h4>
  ...
  @if(Auth::user()->is_admin == "admin")
    ...
    <div class="d-flex align-items-end justify-content-between mt-3">
        ...
        <select class="form-select js-example-basic-single mb-2" id="billType" aria-label="Bill Type" style="width: auto;">
            ...
            @if(!empty($grams))
            @foreach($grams as $gram)
                <option value="{{ $gram->gram_name }}">{{ $gram->gram_name }}-{{$gram->taluka}}-{{$gram->district}}</option>
            @endforeach
            @endif
        </select><br>
    ...
    <div>
        <h4 class="fs-22 fw-semibold ff-secondary mb-4">
            <span id="totalGramBillCount" class="text-primary">{{$allGramCount}}</span>
        </h4>
...
<div class="col-xl-3 col-md-6">
    ...
    <div class="card card-animate">
        <div class="card-body">
            <div class="d-flex align-items-center">
                <div class="flex-grow-1 overflow-hidden">
                    <p class="text-uppercase fw-medium text-muted text-truncate mb-0">
                        Total School Annual Maintenance
                    </p>
                </div>
            </div>
  ...
  <div class="text-center mx-2 mb-4">
            <h4 class="fs-22 fw-semibold ff-secondary" style="margin-top:12px">
                <span id="withoutMaintenanceCount" class="text-primary">{{ $gramsWithoutMaintenance->count() }}</span>
            </h4>
        ...
        <div class="text-center mx-2 mb-4">
            <h4 class="fs-22 fw-semibold ff-secondary" style="margin-top:12px">
                <span id="unpaidMaintenanceCount" class="text-success">{{ $gramsWithUnpaidMaintenance->count() }}</span>
            </h4>
...
@endif
  ...
  <div class="row">
    ...
    <div class="card-header d-flex justify-content-between align-items-center">
        <h5 class="card-title mb-0">Paid Annual Maintenance School</h5>
        <button class="btn btn-primary btn-sm">Total Amount: {{ $paidMaintenanceSum }}</button>
        ...
        <div class="table-responsive">
            <table class="table table-bordered mb-0">
                ...
                <tbody>
                       ...
                       @if(!empty($gramsWithoutMaintenance))
                    @foreach($gramsWithoutMaintenance as $user)
                                     <tr>
    <td>{{ $loop->iteration }}</td>
    <td>{{ $user->gram_name ? $user->gram_name : 'NA' }}</td>
    <td>{{ $user->gram_name && $user->taluka && $user->district && $user->state && $user->pin_code ? $user->gram_name . ' ' . $user->taluka . ' ' . $user->district . ' ' . $user->state . ' ' . $user->pin_code : 'NA' }}</td>
    <td>{{ $user->invoice_no ? $user->invoice_no : 'NA' }}</td>
    <td>{{ $user->maintenance_year ? $user->maintenance_year : 'NA' }}</td>
    <td>{{ $user->maintenance_amount ? $user->maintenance_amount : 'NA' }}</td>
    <td>{{ $user->payment_mode ? $user->payment_mode : 'NA' }}</td>
</tr>
    ...
    <div class="card">
                <div class="card-header align-items-center d-flex">
                    <h4 class="card-title mb-0 flex-grow-1">Pending Payment Annual Maintenance School</h4>
                    <button class="btn btn-primary btn-sm"> Pending Amount: {{ $unpaidMaintenanceSum }}</button>
                ...
                <tbody>
                      @if(!empty($gramsWithUnpaidMaintenance))
                    @foreach($gramsWithUnpaidMaintenance as $user)
                  <tr>
    <td>{{ $loop->iteration }}</td>
    <td>{{ $user->gram_name ? $user->gram_name : 'NA' }}</td>
    <td>{{ $user->gram_name && $user->taluka && $user->district && $user->state && $user->pin_code ? $user->gram_name . ' ' . $user->taluka . ' ' . $user->district . ' ' . $user->state . ' ' . $user->pin_code : 'NA' }}</td>
    <td>{{ $user->invoice_no ? $user->invoice_no : 'NA' }}</td>
    <td>{{ $user->maintenance_year ? $user->maintenance_year : 'NA' }}</td>
     <td>{{ $user->next_maintenance_date ? $user->next_maintenance_date : 'NA' }}</td>
    <td>{{ $user->remaining_amount ? $user->remaining_amount : $user->maintenance_amount }}</td>
    <td>{{ $user->payment_mode ? $user->payment_mode : 'NA' }}</td>
</tr>
