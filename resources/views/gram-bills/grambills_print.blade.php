...
<body>
    <div class=""  style="margin-top:-30px; font-size:13px">
        <span style="margin-left:10px">Export Date And Time : {{ $exportDate }}</span> 
        <span class="right" style="margin-left:150px">{{ $currentUrl }}</span>
    ...
    <div class="container">
        ...
        <div style="padding: 0px 10px;">
            <div style="font-size: 20px; font-weight: bold; text-align: center; margin-top:-10px; font-family: 'Source Sans Pro', sans-serif;" >
                Invoice No. {{$gramBill->invoice_no}}
            </div>
            <div style="display: flex; font-size: 12px; float:right;">
                   <div>
                       <div style="margin-bottom: 15px; margin-top:-20px; margin-left:55px; color: #333;font-size:15px; font-family: 'Source Sans Pro', sans-serif;"><b>Quotation Date</b> - {{$gramBill->quatation_date}}</div>
                        <div style="margin-bottom: 7px; color:#333; font-size:15px; font-family: 'Source Sans Pro', sans-serif; margin-left:95px"><b>Bill Date</b>: {{$gramBill->bill_date}}</div>
                        <div style="margin-bottom: 7px;color: #333; font-size:15px; font-family: 'Source Sans Pro', sans-serif; margin-left:95px"><b>Reference No.</b> {{$gramBill->reference_number}}</div>
                        <div style="color: green; font-weight: bold; font-size:15px; font-family: 'Source Sans Pro', sans-serif; margin-bottom:7px; margin-left:95px"><b>Billing No. {{$gramBill->invoice_no}}</b></div>
                    </div>
                </div>
        </div>
        ...
        <div style="padding: 10px 20px; font-size: 14px; text-align: left; margin-top:-100px; margin-left:-10px">
            <div>
                ...
                <span style=" font-size: 28px; color: #7b1fa2; font-weight: bold font-family: 'Source Sans Pro', sans-serif , 'noto-sans-devanagari';">{{$gramBill->gram}}</span>
            </div>
            <div style="font-size: 14px; color: #333; margin-top: 5px; font-family: 'Source Sans Pro', sans-serif , 'noto-sans-devanagari';">
                Tal- {{$gramBill->taluka}} Dist-{{$gramBill->district}}.
            </div>
        </div>
    ...
    <span style="position: absolute; left: 50%; transform: translateX(-50%); margin-top:-5px;margin-left:-35px">DETAILS</span>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $gramBill->population }}</td>
                </tr>
                <tr>
                    <td>{{ $rowNumber++ }}</td>
                    <td>First Time Bill Amount</td>
                    <td>{{ $gramBill->first_time_bill_amount }}</td>
                </tr>
                <tr>
                    <td>{{ $rowNumber++ }}</td>
                    <td>Maintenance Amount</td>
                    <td>{{ $gramBill->maintenance_amount }}</td>
                </tr>
                <tr>
                    <td>{{ $rowNumber++ }}</td>
                    <td>Next Maintenance Date</td>
                    <td>{{ $gramBill->next_maintenance_date }}</td>
                </tr>
                <tr>
                    <td>{{ $rowNumber++ }}</td>
                    <td>Bill Status</td>
                    <td>{{ $gramBill->bill_status }}</td>
                </tr>
            </tbody>
        </table>
        ...
        <table class="info-table single-header">
            ...
            <tbody>
                ...
                <tr>
                    <td>Paid Amount</td>
                    <td>{{ $gramBill->paid_amount }}</td>
                </tr>
                <tr>
                    <td>Description</td>
                    <td>{{ $gramBill->description }}</td>
                </tr>
                <tr>
                    <td>Payment Mode</td>
                    <td>{{ $gramBill->payment_mode }}</td>
                </tr>
            </tbody>
        </table>
...
<div class="bottom-image" style="position: relative;">
    <img src="{{ public_path('build/images/footer.png')}}" alt="Bottom Image" style=" height: 150px;">
